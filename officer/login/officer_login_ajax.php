...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Find user by email
        $stmt = $conn->prepare("SELECT id, name, email, password, role, electoral_area, status FROM users WHERE email = ?  LIMIT 1");
        ...
        if ($user) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                // Keep backward-compatible electoral_area and also expose as main_community_id
                $_SESSION['electoral_area'] = $user['electoral_area'];
                $_SESSION['main_community_id'] = $user['electoral_area'];
