<?php
// admin/officers/edit_officer.php - Edit Officer Details
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $officer) {
    ...
    if (empty($errors)) {
        try {
            ...
            $check_stmt->execute([$email, $officer_id]);
            ...
            if ($check_stmt->rowCount() > 0) {
                ...
                $update_stmt->execute([$name, $email, $phone, $status, $main_community_id, $smaller_community_id, $suburb_id, $cottage_id, $officer_id]);
...
$headerActionButtons = [
    [
        'icon' => 'fas fa-eye',
        'label' => 'View Officer',
        'href' => './view_officer.php?id=' . $officer_id,
...
<head>
    ...
    <title>Edit Officer - Admin Dashboard</title>
...
<body class="bg-slate-50 min-h-screen font-sans">
    ...
    <main class="lg:ml-64 min-h-screen transition-all duration-300">
        <?php renderAdminHeader('Edit Officer', 'Update officer information', $headerActionButtons); ?>
        ...
        <div class="p-4 sm:p-6">
                    ...
                    <form method="POST" action="" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" value="<?php echo htmlspecialchars($officer['name']); ?>" class="w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                            </div>

                            <!-- Email -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Email Address *</label>
                                <input type="email" name="email" value="<?php echo htmlspecialchars($officer['email']); ?>" class="w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                            </div>

                            <!-- Phone -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Phone Number *</label>
                                <input type="tel" name="phone" value="<?php echo htmlspecialchars($officer['phone']); ?>" class="w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="status" class="w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                                    <option value="active" <?php echo $officer['status'] === 'active' ? 'selected' : ''; ?>>Active</option>
                                    <option value="inactive" <?php echo $officer['status'] === 'inactive' ? 'selected' : ''; ?>>Inactive</option>
                                </select>
                            </div>
                        </div>
                                ...
                                <div>
                                    ...
                                    <select name="main_community_id" id="main_community" class="w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                                        ...
                                        <?php foreach ($communities as $community): ?>
                                            <option value="<?php echo $community['id']; ?>" <?php echo $officer['main_community_id'] == $community['id'] ? 'selected' : ''; ?>>
                                ...
                                <div>
                                    ...
                                    <select name="smaller_community_id" id="smaller_community" class="w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                        ...
                                        <?php foreach ($smaller_communities as $smaller_community): ?>
                                            <option value="<?php echo $smaller_community['id']; ?>" <?php echo $officer['smaller_community_id'] == $smaller_community['id'] ? 'selected' : ''; ?>>
                                ...
                                <div>
                                    ...
                                    <select name="suburb_id" id="suburb" class="w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                        ...
                                        <?php foreach ($suburbs as $suburb): ?>
                                            <option value="<?php echo $suburb['id']; ?>" <?php echo $officer['suburb_id'] == $suburb['id'] ? 'selected' : ''; ?>>
                                ...
                                <div>
                                    ...
                                    <select name="cottage_id" id="cottage" class="w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                        ...
                                        <?php foreach ($cottages as $cottage): ?>
                                            <option value="<?php echo $cottage['id']; ?>" <?php echo $officer['cottage_id'] == $cottage['id'] ? 'selected' : ''; ?>>
                        ...
                        <div class="flex justify-end space-x-4">
                            <a href="./view_officer.php?id=<?php echo $officer_id; ?>" class="px-6 py-2 text-sm font-medium text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 transition">
                                Cancel
                            </a>
            ...
            <?php else: ?>
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 text-center">
                    ...
                    <p class="text-gray-600 mb-4">The officer you're trying to edit doesn't exist or has been removed.</p>
