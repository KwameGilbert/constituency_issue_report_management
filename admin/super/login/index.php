...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) && empty($password)) {
        ...
        // Check if supervisor exists and is an MP (super_admin)
        $query = "SELECT id, email, password, name, position, status FROM supervisors 
                 WHERE email = ? AND position = 'mp' LIMIT 1";
        ...
        if ($result->num_rows === 0) {
            $error_message = "No MP account found with this email address.";
            ...
            if ($supervisor['status'] === 'inactive') {
                $error_message = "Your account is currently inactive. Please contact system administrator.";
                $error_type = 'inactive';
            } elseif ($supervisor['status'] === 'suspended') {
                $error_message = "Your account has been suspended. Please contact system administrator.";
                $error_type = 'suspended';
            } elseif (!password_verify($password, $supervisor['password'])) {
                $error_message = "Incorrect password. Please try again.";
                $error_type = 'invalid_password';
            } else {
                // Set session variables
                $_SESSION['admin_id'] = $supervisor['id'];
                $_SESSION['admin_name'] = $supervisor['name'];
                $_SESSION['email'] = $supervisor['email'];
                $_SESSION['role'] = 'super_admin'; // Role for system purposes
                $_SESSION['position'] = 'mp'; // Actual position from DB
                
                // Update last login timestamp in supervisors table
                $update_query = "UPDATE supervisors SET last_login = NOW() WHERE id = ?";
                $update_stmt = $conn->prepare($update_query);
                $update_stmt->bind_param("i", $supervisor['id']);
                $update_stmt->execute();
                $update_stmt->close();
                
                // Redirect to MP/Super Admin dashboard
                header("Location: ../dashboard/");
                exit();
            }
        }
...
<head>
    ...
    <title>MP Admin Login - Constituency Management System</title>
    ...
    <div class="flex h-screen">
        ...
        <div class="hidden md:flex md:w-1/2 flex-col ghana-colors justify-center items-center relative">
            ...
            <div class="relative z-10 flex flex-col items-center text-white">
                ...
                <h2 class="text-2xl font-semibold mb-6 text-center">MP Administration Portal</h2>
                <p class="max-w-md text-center px-8 text-lg">Access your constituency management dashboard to oversee
                    projects, issues, and staff</p>
            </div>
        ...
        <div class="w-full md:w-1/2 flex items-center justify-center">
            <div class="max-w-md w-full p-8">
                <div class="text-center mb-10">
                    ...
                    <h2 class="text-3xl font-bold text-gray-800">MP Admin Login</h2>
                    <p class="text-gray-600 mt-2">Access your constituency management dashboard</p>
                </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            ...
                            <p class="mt-2 text-sm">
                                <a href="mailto:support@parliament.gh" class="underline hover:text-gray-900">Contact
                                    IT support for assistance</a>
                            </p>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <form action="<?= $_SERVER['PHP_SELF']; ?>" method="POST" class="space-y-6">
                    ...
                    <div>
                        <button type="submit"
                            ...
                            <i class="fas fa-sign-in-alt mr-2"></i> Sign in to MP Admin Portal
                        </button>
                    </div>
                </form>
                ...
                <div class="mt-6 text-center">
                    ...
                    <p class="text-sm font-medium text-green-600">support@parliament.gh | 030 222 3344</p>
                </div>

                <!-- Footer with Government Info -->
                <div class="mt-8 text-center text-gray-600 text-sm">
                    <p>Government of Ghana</p>
                    <p class="mt-1">Parliament of Ghana</p>
