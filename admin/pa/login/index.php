...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) && empty($password)) {
        ...
        // Check if PA exists
        $query = "SELECT id, name, email, password, status FROM personal_assistants 
                 WHERE email = ? LIMIT 1";
        ...
        if ($result->num_rows === 0) {
            ...
            if ($pa['status'] === 'inactive') {
                ...
                $error_type = 'inactive';
            } elseif ($pa['status'] === 'suspended') {
                $error_message = "Your account has been suspended. Please contact the administrator.";
                $error_type = 'suspended';
            } elseif (!password_verify($password, $pa['password'])) {
                $error_message = "Incorrect password. Please try again.";
                $error_type = 'invalid_password';
            } else {
                // Set session variables
                $_SESSION['pa_id'] = $pa['id'];
                $_SESSION['pa_name'] = $pa['name'];
                $_SESSION['email'] = $pa['email'];
                $_SESSION['role'] = 'pa';
                
                // Update last login timestamp
                $update_query = "UPDATE personal_assistants SET last_login = NOW() WHERE id = ?";
                $update_stmt = $conn->prepare($update_query);
                $update_stmt->bind_param("i", $pa['id']);
                $update_stmt->execute();
                $update_stmt->close();
                
                // Redirect to PA dashboard
                header("Location: ../dashboard/");
                exit();
            }
        }
...
<head>
    ...
    <title>Personal Assistant Login - Constituency Management System</title>
    ...
    <div class="flex h-screen">
        ...
        <div class="hidden md:flex md:w-1/2 flex-col ghana-colors justify-center items-center relative">
            ...
            <div class="relative z-10 flex flex-col items-center text-white">
                ...
                <h2 class="text-2xl font-semibold mb-6 text-center">Personal Assistant Portal</h2>
                <p class="max-w-md text-center px-8 text-lg">Managing projects, issues and employment opportunities for
                    better constituency governance</p>
            </div>
        ...
        <div class="w-full md:w-1/2 flex items-center justify-center">
            <div class="max-w-md w-full p-8">
                <div class="text-center mb-10">
                    ...
                    <h2 class="text-3xl font-bold text-gray-800">Personal Assistant Login</h2>
                ...
                <form action="<?= $_SERVER['PHP_SELF']; ?>" method="POST" class="space-y-6">
                    ...
                    <div>
                        <button type="submit"
                            ...
                            <i class="fas fa-sign-in-alt mr-2"></i> Sign in to PA Portal
                        </button>
                    </div>
                </form>
