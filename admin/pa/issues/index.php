    ...
    <div class="p-4 mt-14">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            ...
            <div class="mt-4 md:mt-0">
                <button type="button" id="toggleFilters" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
        ...
        <div id="filtersContainer" class="bg-white rounded-lg shadow-md p-4 mb-6 <?= !empty($status) || !empty($severity) || !empty($electoral_area) || !empty($search) || !empty($start_date) || !empty($end_date) ? '' : 'hidden' ?>">
            ...
            <form action="" method="GET" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">
                    ...
                    <div>
                        ...
                        <select name="severity" id="severity" class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">
                    ...
                    <div>
                        <label for="electoral_area" class="block text-sm font-medium text-gray-700 mb-1">Electoral Area</label>
                        <select name="electoral_area" id="electoral_area" class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">
                    ...
                    <div>
                        ...
                        <input type="text" name="search" id="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search titles, descriptions, locations..." class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date" value="<?= htmlspecialchars($start_date) ?>" class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">
                    </div>

                    <div>
                        <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">Date To</label>
                        <input type="date" name="end_date" id="end_date" value="<?= htmlspecialchars($end_date) ?>" class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">
                    </div>

                    <!-- Sort -->
                    <div>
                        <label for="sort" class="block text-sm font-medium text-gray-700 mb-1">Sort By</label>
                        <select name="sort" id="sort" class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">
                            <option value="newest" <?= $sort === 'newest' ? 'selected' : '' ?>>Newest First</option>
                            <option value="oldest" <?= $sort === 'oldest' ? 'selected' : '' ?>>Oldest First</option>
                            <option value="severity_high" <?= $sort === 'severity_high' ? 'selected' : '' ?>>Severity (High to Low)</option>
                            <option value="severity_low" <?= $sort === 'severity_low' ? 'selected' : '' ?>>Severity (Low to High)</option>
            ...
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <?php if (!empty($search)): ?>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        Search: <?= htmlspecialchars(substr(trim($search, '%'), 0, 20)) ?><?= strlen(trim($search, '%')) > 20 ? '...' : '' ?>
                    </span>
                <?php endif; ?>
                
                <?php if (!empty($start_date) || !empty($end_date)): ?>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                        Date: <?= !empty($start_date) ? $start_date : 'Any' ?> to <?= !empty($end_date) ? $end_date : 'Any' ?>
                    </span>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php while ($issue = $result->fetch_assoc()): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4">
                                        <div>
                                            <a href="view.php?id=<?= $issue['id'] ?>" class="text-sm font-medium text-gray-900 hover:text-green-600">
                                                <?= htmlspecialchars($issue['title']) ?>
                                            </a>
                                            <div class="text-xs text-gray-500 mt-1">
                                                ID: <?= $issue['id'] ?>
                                                <span class="ml-2">
                                                    <i class="fas fa-users text-gray-400"></i> <?= number_format($issue['people_affected'] ?? 0) ?> people affected
                                                </span>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        ...
                                        <a href="update-status.php?id=<?= $issue['id'] ?>&status=under_review" class="text-blue-600 hover:text-blue-900 mr-3">
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700">
                                    Showing <span class="font-medium"><?= min(($page - 1) * $records_per_page + 1, $total_records) ?></span> to 
            ...
            <?php else: ?>
                <div class="p-6 text-center">
                    ...
                    <?php if (!empty($status) || !empty($severity) || !empty($electoral_area) || !empty($search) || !empty($start_date) || !empty($end_date)): ?>
                        <div class="mt-4">
                            <a href="index.php" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
